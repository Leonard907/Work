...
test1 = TestCase (assertEqual "Desired action" v (Move (('b', 1), ('c', 2))))
    where v = minimaxABFromTree $ highFirst testTree1

test2 :: Test
test2 = TestCase (assertEqual "Desired action" v (Move (('a', 1), ('k', 2))))
    where v = minimaxABFromTree $ highFirst testTree2

abMinimax :: Spec
abMinimax = fromHUnitTest $ TestList
    [ TestLabel "testAlphaBetaCorrectness" test1
    , TestLabel "testAlphaBetaCorrectness" test2
    ]
