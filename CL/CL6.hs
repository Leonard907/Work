...
import Data.List (sortOn, delete, intercalate, intersperse, nub)
...
data Colour = Red | Green | Amber       deriving (Ord,  Eq, Show)
...
adjacentCitiesNotSameColour paint = and[ or [not (paint a c && paint b c)] | a <- cities, b <- cities, adj a b, c <- colours ]
...
--------------------------
data Paint = Paint City Colour deriving (Ord, Eq, Show)
...
formalAdjacentCitiesNotSameColour = And [ Or [N (Paint a c), N (Paint b c)] | a <- cities, b <- cities, adj a b, c <- colours ] 

----------------------------
(<<) :: Eq a => [Clause a] -> Literal a -> [Clause a] 
cs << x = [ Or (delete (neg x) ys) | Or ys <- cs, not $ x `elem` ys ] -- DPLL backtracking process
...
simple (And cs) =
  let search cs = case cs of
        ...
        Or (x : _) : _  ->
          [     x : m | m <- search $ cs << x   ] -- Remove one matching Clause
          ++
          [ neg x : m | m <- search $ cs << neg x] -- Remove the negation of literal from that Clause
  in
   search (canonical cs)
...
toSquare n vs =
  let rows    = [0..n-1]
      ...
      digits  = [1..n]
  in
   Square [ [ d | c <- columns, d <- digits,
              P(r,c,d) `elem` vs]| r <- rows]
--
...
dpll (And cs) =
  ...
  in
   models (canonical cs)
  where prioritise clauses = sortOn (length . sep) clauses
        sep (Or clause) = clause

-- this is for you to play with faster versions
speedy :: Ord a => Form a -> [Val a]
speedy (And css) =
  let models css = case prioritise css of
        []            -> [[]]     -- no clauses; trivial solution
        Or []  : _    -> []       -- empty clause; no models
        Or [x] : _    -> [     x : m | m <- models $ css << x   ] -- unit clause
        Or (x : _) : _  ->
          [     x : m | m <- models $ css << x   ]
          ++
          [ neg x : m | m <- models $ css << neg x]
  in
    models (canonical css)
  where prioritise clauses = nub ((units clauses) ++ clauses)
        units clauses = filter (\x -> length (sep x) == 1 ) clauses
        sep (Or clause) = clause
    ...
    if length p == 81
      ...
      do putStrLn "\nSudoku Problem:"
         ...
         let report =
               ...
               [s]  -> ["Unique Solution"]
                       ++
                       [(pretty.showEntries) (head solutions)]
               s:ss -> ["Multiple Solutions : showing five"]
                       ++
