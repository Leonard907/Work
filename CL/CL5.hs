...
reach2 x z = some people (\y -> reach1 x y && reach1 y z)
reach3 :: Person -> Person -> Bool
reach3 x z = some people (\y -> reach1 x y && reach2 y z)
...
check entered =
  ...
  and [ or [entered i j k | i <- [ii..ii+2], j <- [jj..jj+2]]
      | ii <- [1,4,7], jj <- [1,4,7], k <- [1..9]]
