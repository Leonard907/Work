...
startingBoard :: Board
startingBoard = b where (b, _, _) = graphFromEdges startingEdges
...
positionAheadGame = fromJust
    $ performAction (fromJust $ performAction startingGame actX) actY
  where
    actX, actY :: Action
    actX =
        let c = currentCell (currentPlayer startingPlayers) in Move (stepTop c)
    actY =
        let c = currentCell (previousPlayer startingPlayers)
        in  Move (stepLeft c)

-- The utility function should give a higher value to 'positionAheadGame'.
testUtilityPositionAhead :: Test
testUtilityPositionAhead = TestCase
    (assertLessThan "positionAheadGame"
                    (utility startingGame)
                    (utility positionAheadGame)
    )

-- Game where X moves one position to the right and Y places a wall in front of it.
wallInFrontGame :: Game
wallInFrontGame = fromJust
    $ performAction (fromJust $ performAction startingGame actX) actY
  where
    actX, actY :: Action
    actX =
        let c = currentCell (currentPlayer startingPlayers)
        in  Move (stepRight c)
    actY =
        let c = currentCell (currentPlayer startingPlayers)
        in  Place (wallTop c)
...
testUtilityWallInFront = TestCase
    (assertLessThan "wallInFrontGame"
                    (utility wallInFrontGame)
                    (utility startingGame)
    )

-- All unit tests together.
utilityUnitTests :: Spec
utilityUnitTests = fromHUnitTest $ TestList
